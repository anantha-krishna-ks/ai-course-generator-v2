  ...
  return (
    ...
    <Popover open={forceOpen || undefined}>
      <PopoverTrigger asChild>
        ...
        >
          <div className={cn(
            "flex-1 h-px transition-all duration-300 ease-out",
            isDragOver ? "bg-[hsl(217,91%,60%)]/50 opacity-100" : "bg-foreground/15",
            !isDragOver && !forceOpen && "opacity-0 group-hover/add:opacity-100",
            forceOpen && "opacity-100"
          )} />
          <div className={cn(
            "mx-3 rounded-full border flex items-center justify-center bg-background/50 hover:bg-background hover:border-[hsl(217,91%,60%)]/50 hover:scale-110 transition-all duration-300 ease-out",
            isDragOver
              ? "w-9 h-9 border-[hsl(217,91%,60%)] border-dashed bg-[hsl(217,91%,60%)]/5 scale-110 opacity-100"
              : "w-7 h-7 border-foreground/20",
            !isDragOver && !forceOpen && "opacity-0 group-hover/add:opacity-100",
            forceOpen && "opacity-100"
          )}>
            <Plus className={cn("transition-all duration-300 ease-out", isDragOver ? "w-4 h-4 text-[hsl(217,91%,60%)]" : "w-3.5 h-3.5 text-foreground/40")} />
          </div>
          <div className={cn(
            "flex-1 h-px transition-all duration-300 ease-out",
            isDragOver ? "bg-[hsl(217,91%,60%)]/50 opacity-100" : "bg-foreground/15",
