        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
          ...
          <div className="space-y-3">
            <Textarea
              ...
              rows={3}
            />
            {options.exclusionsDocuments.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {options.exclusionsDocuments.map((doc, i) => (
                  <Badge
                    key={i}
                    variant="secondary"
                    className="gap-1.5 pr-1.5 text-xs font-normal h-7"
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => update({ exclusionsDocuments: options.exclusionsDocuments.filter((_, idx) => idx !== i) })}
                      className="ml-1 rounded-full p-0.5 hover:bg-muted-foreground/20 transition-colors"
                    >
                      <X className="w-3 h-3" />
                    </button>
      ...
      <Dialog open={infoDialog !== null} onOpenChange={(open) => !open && setInfoDialog(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {infoDialog === "guidelines"
                ? "Use these as a starting point when writing your own."
                : "Common things you might want the AI to avoid."}
            </DialogDescription>
          </DialogHeader>
            ...
            {(infoDialog === "guidelines" ? GUIDELINES_EXAMPLES : EXCLUSIONS_EXAMPLES).map(
              (item, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-foreground leading-relaxed">
                  <span
                    className="mt-[7px] w-1.5 h-1.5 rounded-full bg-primary shrink-0"
                    aria-hidden="true"
                  />
                  <span>{item}</span>
                </li>
    ...
    <div className="flex items-center gap-2.5">
      ...
      {onInfoClick && (
        <button
          ...
          className="ml-1 w-6 h-6 rounded-full inline-flex items-center justify-center border border-border bg-background text-muted-foreground hover:text-primary hover:border-primary/30 hover:bg-primary/5 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
        >
